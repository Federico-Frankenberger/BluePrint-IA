...
| Change request doesn't match any `out_of_scope` entry, but relates to a confirmed `RN-##`/`RF-##`/`HU-##` | `contradicts_out_of_scope: false`; run the script seeded from that id to report the blast radius anyway (even in-scope changes can ripple downstream). |
...
2. Load `state/prd-state.json`; check the description against every `out_of_scope` entry (apply the matching Decision Gate).
...
Each run ends with the updated `state/change-requests-state.json` on disk and a chat report: contradiction verdict (with the exact PRD out-of-scope quote when applicable), the affected items per pipeline stage exactly as `find-affected.sh` reported them, and the new `CR-##` id. This skill never edits any other `*-state.json`, never invokes another skill automatically, and never states or implies that the change is approved.
